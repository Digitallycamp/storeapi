...
const prodcutController = async (req, res) => {
	...
	const { title, description, amount, image, ratings, category } = req.body;
	...
	try {
		...
		const text =
			'INSERT INTO products(product_id, title, description, amount, image, ratings, category, userid) VALUES( $1, $2, $3, $4, $5, $6, $7, $8) RETURNING *';
		const values = [
			product.productId,
			product.title,
			product.description,
...
const getProductById = async (req, res) => {
	...
	try {
		...
		const findProd = prodcuctById.rows.find(
...
const updateProuct = async (req, res) => {
	...
	try {
		...
		res
			.status(201)
			.json({ message: 'product updated sucessfully', data: newProducts.rows });
	} catch (error) {
		res.status(500).json({
			message: error.message,
		});
...
const deleteProduct = async (req, res) => {
	...
	try {
		...
		if (result.rowCount === 0) {
			return res
				.status(404)
				.json({ message: `Product with ${productid} have been deleted!` });
		}
		res.status(204).send();
	} catch (error) {
		res.status(500).json({ message: error.message });
